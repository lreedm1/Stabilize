...
VERSION="20260813-mobile-hevc-v35-1"
...
METADATA="scripts/mobile-hevc-v35.json"
WORK="${RUNNER_TEMP:-/tmp}/stabilize-mobile-hevc-v35"
mkdir -p "$WORK"
...
if [[ ! -s "$OUTPUT" || ! -s "$METADATA" ]]; then
  ...
  ffmpeg -hide_banner -v error -y \
    -i "$SOURCE" \
    -t 5.0 -an \
    -vf "scale=1080:1920:force_original_aspect_ratio=increase:flags=lanczos,crop=1080:1920,setsar=1,minterpolate=fps=60:mi_mode=mci:mc_mode=aobmc:me_mode=bidir:vsbmc=1,format=yuv420p" \
    -c:v libx265 -profile:v main -preset medium -crf 16 \
...
if not 1_250_000 < len(data) < 10_000_000:
...
metadata = {
    ...
    "h264Asset": "/scenes/mobile-forest-stream-video-v24-native-1080.mp4",
    "videoBytes": len(data),
    "videoSha256": sha256,
    ...
    "quality": "native-video-hevc-1080x1920-60fps",
    "fallbackWidth": 2160,
    "fallbackHeight": 3840,
    "fallbackFps": 24,
    "fallbackQuality": "native-video-h264-2160x3840-24fps",
}
