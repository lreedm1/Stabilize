...
# The native finalizer derives resolution from metadata. Seed the two existing
# responsive poster descriptors for the first generation run; subsequent runs
# are regenerated by the normal historical pipeline before the finalizer.
page_path = root / "src/page.js"
page = page_path.read_text()
page = page.replace(f"{POSTER_ASSET} 1080w", f"{POSTER_ASSET} {WIDTH}w")
page_path.write_text(page)

# Run the small coherent visual finalizer at the very end of the existing
# native regression finalizer. Keeping package.json untouched avoids changing
# the global pipeline contract used by unrelated tests.
