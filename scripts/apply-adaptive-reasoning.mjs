...
await update("test/worker.test.mjs", (source) => {
  ...
  if (!text.includes('test("complex decisions use maximum reasoning"')) {
    const integrationTest = `test("complex decisions use maximum reasoning", async () => {
...
`;
  ...
  if (
    ...
    throw new Error(
      "Adaptive reasoning did not align the lightweight-turn test",
    );
  }
  if (
    !text.includes(
