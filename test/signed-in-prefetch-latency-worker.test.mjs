...
test("a prefetched signed-in chat skips the account-memory Durable Object read", async () => {
  ...
  const context = await fetchContext(setup, account);
  assert.match(context.token, /^[A-Za-z0-9_-]+\.[A-Za-z0-9_-]+$/);
  assert.equal(context.expiresInSeconds, 900);
...
test("an invalid context token safely falls back to account memory", async () => {
  const setup = createEnv();
  const account = await identity(setup.env, "invalid-prefetch-token-user");
  setup.sessions.seed(account.objectName, {
    summary: "CURRENT DURABLE CONTEXT",
    generation: 3,
  });
  const originalFetch = globalThis.fetch;
  let providerBody;
  globalThis.fetch = async (_input, init) => {
    providerBody = JSON.parse(init.body);
    return responseWithText("Use the first small step.");
  };
  ...
  try {
    ...
    assert.equal(setup.sessions.reads(account.objectName), 1);
    assert.match(JSON.stringify(providerBody.input), /CURRENT DURABLE CONTEXT/);
    assert.equal(setup.billing.generation(account.objectName), 3);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
...
test("deleting memory revokes an older prefetched token", async () => {
  const setup = createEnv();
