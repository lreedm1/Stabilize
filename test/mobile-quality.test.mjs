...
test("portrait mobile draws water through a canvas without media autoplay", async () => {
  const [pageSource, mobileStyles, canvasClient, videoClient, poster, sprite] =
    await Promise.all([
      ...
      readFile(
        new URL(
          "../public/scenes/mobile-forest-stream-v24-native-1080.webp",
          import.meta.url,
        ),
      ),
      readFile(
  ...
  assert.equal(
    [...pageSource.matchAll(/mobile-forest-stream-v24-native-1080\.webp 1080w/g)]
      .length,
    2,
  );
...
test("portrait mobile serves the native selected forest scene", async () => {
  const [pageSource, clientSource, responderSource, video, poster] =
    await Promise.all([
      ...
      readFile(new URL("../public/scenes/mobile-forest-stream-video-v24-native-1080.mp4", import.meta.url)),
  ...
  assert.match(responderSource, /MOBILE_VIDEO_BYTES = 2_371_524/);
  assert.match(responderSource, /69dd547594f86fb80f643fa7c823d076c414a630d9a5a53504b6d5f930b95ffc/);
});
// ai-enhanced-mobile-4k-v23-test-end

// original-mobile-image-v21-quality-test-start
test("portrait mobile plays the selected forest-stream scene through the native route", async () => {
